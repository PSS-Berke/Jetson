...
type WizardAction =
  ...
  | {
      ...
      payload: { name: string; isCustom: boolean };
    }
  | { type: "SET_CUSTOM_PROCESS_TYPE_FIELDS"; payload: CustomFormField[] }
  | {
      type: "SET_CAPABILITIES";
      payload: Record<string, MachineCapabilityValue>;
    }
  | {
      type: "SET_CAPABILITY";
      payload: { field: string; value: MachineCapabilityValue };
    }
  ...
  | {
      ...
      payload: { id: string; field?: Partial<FormBuilderField> };
    }
...
function wizardReducer(state: WizardState, action: WizardAction): WizardState {
  switch (action.type) {
    case "SET_STEP":
      return {
        ...state,
        currentStep: action.payload as WizardState["currentStep"],
      };
    ...
    case "ADD_MACHINE_VARIABLE":
      return {
        ...state,
        machineVariables: [...state.machineVariables, action.payload],
      };
    ...
    case "REMOVE_MACHINE_VARIABLE":
      return {
        ...state,
        machineVariables: state.machineVariables.filter(
    ...
    case "ADD_FORM_BUILDER_FIELD":
      return {
        ...state,
        formBuilderFields: [...state.formBuilderFields, action.payload],
      };
    ...
    case "REMOVE_FORM_BUILDER_FIELD":
      return {
        ...state,
        formBuilderFields: state.formBuilderFields.filter(
  ...
  const validateStep3 = useCallback((): boolean => {
    ...
    if (
      ...
      errors.customProcessTypeFields =
        "Please add at least one field to your custom process type";
    }

    // Validate custom field values if any custom fields exist
    if (state.customProcessTypeFields.length > 0) {
      const requiredFields = state.customProcessTypeFields.filter(
        (f) => f.required,
      );
      const missingFields = requiredFields.filter(
        (field) => !state.capabilities[field.id],
      );
    ...
    if (state.machineVariables && state.machineVariables.length > 0) {
      const requiredVariables = state.machineVariables.filter(
        (v) => v.required,
      );
  ...
  const validateStep4 = useCallback((): boolean => {
    ...
    state.rules.forEach((rule, index) => {
      ...
      if (rule.conditions.length === 0) {
        errors[`rule_${index}_conditions`] =
          "At least one condition is required";
      }
      if (
        rule.outputs.speed_modifier <= 0 ||
        rule.outputs.speed_modifier > 200
      ) {
        errors[`rule_${index}_speed`] =
          "Speed modifier must be between 1 and 200";
      }
      if (rule.outputs.people_required <= 0) {
        errors[`rule_${index}_people`] =
          "People required must be greater than 0";
      }
    });
      ...
      switch (step) {
        ...
        case 3:
          ...
          if (state.isCustomProcessType) {
            ...
            if (
              !state.customProcessTypeName.trim() ||
              state.customProcessTypeFields.length === 0
            )
              return false;
