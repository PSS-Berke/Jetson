  ...
  switch (processType.toLowerCase()) {
    case "insert":
      if (!requirement.paper_size) {
        errors.push(
          `Row ${row}: Insert requires paper_size (6x9, 6x12, 9x12, 10x13, 12x15, #10, 11x17)`,
        );
      }
      break;
    ...
    case "sort":
      if (!requirement.sort_type) {
        errors.push(
          `Row ${row}: Sort requires sort_type (Standard Sort, Presort, EDDM, Full Service)`,
        );
      }
      if (!requirement.paper_size) {
    ...
    case "inkjet":
      if (!requirement.print_coverage) {
        errors.push(
          `Row ${row}: Inkjet requires print_coverage (Black & White, Full Color, Spot Color)`,
        );
      }
      if (!requirement.paper_size) {
    ...
    case "label/apply":
      if (!requirement.application_type) {
        errors.push(
          `Row ${row}: Label/Apply requires application_type (Label Application, Affix, Wafer Seal)`,
        );
      }
      if (!requirement.label_size) {
        errors.push(
          `Row ${row}: Label/Apply requires label_size (1x2.625, 2x3, 3x5, 4x6, Custom)`,
        );
      }
      if (!requirement.paper_size) {
    ...
    case "fold":
      if (!requirement.fold_type) {
        errors.push(
          `Row ${row}: Fold requires fold_type (Half Fold, Tri-fold, Z-fold, Double Parallel, Roll Fold)`,
        );
      }
      if (!requirement.paper_stock) {
        errors.push(
          `Row ${row}: Fold requires paper_stock (20# Bond, 24# Bond, 60# Text, 80# Text, 100# Text, Cardstock)`,
        );
      }
      if (!requirement.paper_size) {
        errors.push(
          `Row ${row}: Fold requires paper_size (8.5x11, 8.5x14, 11x17, 12x18, Custom)`,
        );
      }
      break;
    ...
    case "laser":
      if (!requirement.print_type) {
        errors.push(
          `Row ${row}: Laser requires print_type (Simplex (1-sided), Duplex (2-sided))`,
        );
      }
      if (!requirement.paper_stock) {
        errors.push(
          `Row ${row}: Laser requires paper_stock (20# Bond, 24# Bond, 60# Cover, 80# Cover)`,
        );
      }
      if (!requirement.paper_size) {
        errors.push(
          `Row ${row}: Laser requires paper_size (Letter, Legal, Tabloid, 11x17)`,
        );
      }
      if (!requirement.color) {
        errors.push(
          `Row ${row}: Laser requires color (Black & White, Full Color)`,
        );
      }
      break;
    ...
    case "hp press":
      if (!requirement.print_type) {
        errors.push(
          `Row ${row}: HP Press requires print_type (Simplex, Duplex)`,
        );
      }
      if (!requirement.paper_stock) {
        errors.push(
          `Row ${row}: HP Press requires paper_stock (80# Text, 100# Text, 80# Cover, 100# Cover, 12pt Cardstock)`,
        );
      }
      if (!requirement.paper_size) {
        errors.push(
          `Row ${row}: HP Press requires paper_size (12x18, 13x19, Custom)`,
        );
      }
      break;
  }
    ...
    reader.onload = (e) => {
      try {
        ...
        jsonData.forEach((row: any, index: number) => {
          ...
          const sort_type =
            String(findColumnValue(row, "sort_type") || "").trim() || undefined;
