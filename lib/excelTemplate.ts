...
export function downloadExcelTemplate() {
  ...
  const instructions = [
    ...
    ['Production Quantity', 'Required', 'Number of units produced (must be positive)'],
    ['Date', 'Optional', 'Production date in YYYY-MM-DD or MM/DD/YYYY format. If empty, uses today\'s date.'],
    ['Notes', 'Optional', 'Any additional notes about the production entry'],
    [''],
    ['Important Notes:'],
    [''],
    ['1. The job number must exist in your system', 'Missing job numbers will show as errors'],
    ['2. Production quantity must be a positive number', 'Negative or zero values will be rejected'],
    ['3. Date format should be YYYY-MM-DD or MM/DD/YYYY', 'Other formats may cause errors'],
    ['4. Remove these example rows before importing', 'Only keep your actual data'],
    ['5. Do not change the column headers', 'The system expects these exact names'],
    [''],
    ['Tips:'],
    [''],
    ['- You can have as many rows as needed', 'The system will process all valid entries'],
    ['- Invalid rows will be highlighted in the preview', 'You can remove them before final upload'],
    ['- The system checks for duplicate entries', 'Warnings will be shown but upload is allowed'],
