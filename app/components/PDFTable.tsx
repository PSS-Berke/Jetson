...
export default function PDFTable({ timeRanges, jobProjections }: PDFTableProps) {
  return (
    <div className="pdf-table-container mt-6">
      <h3 className="font-semibold text-[var(--text-dark)] mb-4">Job Projections</h3>
      <table className="pdf-table w-full border-collapse">
        ...
        <tbody>
          {jobProjections.length === 0 ? (
            <tr>
              <td colSpan={8 + timeRanges.length} className="pdf-table-cell text-center text-[var(--text-light)]">
            ...
            jobProjections.map((projection) => {
              ...
              return (
                <tr key={job.id}>
                  <td className="pdf-table-cell font-medium">{job.job_number}</td>
                  <td className="pdf-table-cell">{job.client?.name || 'Unknown'}</td>
                  ...
                  <td className="pdf-table-cell max-w-xs truncate">{job.description || 'N/A'}</td>
                  <td className="pdf-table-cell text-center font-medium">{job.quantity.toLocaleString()}</td>
