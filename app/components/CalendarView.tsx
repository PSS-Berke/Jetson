...
const MemoizedDateHeader = memo(
  ({
    ...
    console.log(
      `[CalendarView] DateHeader for ${dateKey}, has summary: ${!!summary}`,
    );
      ...
      ? [
          {
            key: "insert",
            label: "Insert",
            count: summary.processTypeCounts.insert,
            color: getProcessTypeColor("insert"),
          },
          {
            key: "sort",
            label: "Sort",
            count: summary.processTypeCounts.sort,
            color: getProcessTypeColor("sort"),
          },
          {
            key: "inkjet",
            label: "Inkjet",
            count: summary.processTypeCounts.inkjet,
          ...
          },
          {
            key: "fold",
            label: "Fold",
            count: summary.processTypeCounts.fold,
            color: getProcessTypeColor("fold"),
          },
          {
            key: "laser",
            label: "Laser",
            count: summary.processTypeCounts.laser,
            color: getProcessTypeColor("laser"),
          },
          {
            key: "hpPress",
            label: "HP Press",
            count: summary.processTypeCounts.hpPress,
            color: getProcessTypeColor("hpPress"),
          },
        ].filter((badge) => badge.count > 0)
      : [];

    console.log(
      `[CalendarView] DateHeader rendering for ${dateKey}, expandedBadges:`,
      Array.from(expandedBadges),
    );
        ...
        {processTypeBadges.length > 0 && (
          <div className="process-type-badges">
            {processTypeBadges.map((badge) => {
              ...
              const badgeJobs = isExpanded
                ? getJobsForDateAndProcess(date, badge.key)
                : [];

              console.log(
                `[CalendarView] Rendering badge ${badge.key} for ${dateKey}`,
              );
              console.log(`[CalendarView]   - badgeId: ${badgeId}`);
              console.log(`[CalendarView]   - isExpanded: ${isExpanded}`);
              console.log(
                `[CalendarView]   - expandedBadges.has('${badgeId}'): ${expandedBadges.has(badgeId)}`,
              );
              console.log(
                `[CalendarView]   - badgeJobs.length: ${badgeJobs.length}`,
              );
                ...
                <div key={badge.key} className="process-badge-container">
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      console.log(
                        `[CalendarView] Badge clicked: ${badge.key} on ${dateKey}`,
                      );
                      toggleBadge(dateKey, badge.key);
                    }}
                  >
                    <span>
                      {badge.label}: {formatNumber(badge.count)}
                    </span>
                  ...
                  {isExpanded &&
                    (() => {
                      console.log(
                        `[CalendarView] Rendering job-list div for ${badgeId}, isExpanded=${isExpanded}, jobs:`,
                        badgeJobs,
                      );
                      return (
                        <div
                        ...
                        >
                          {badgeJobs.length > 0 ? (
                            badgeJobs.map((job, index) => {
                              console.log(
                                `[CalendarView] Mapping job ${index}:`,
                                job.job_number,
                              );
                              return (
                                <div
  ...
  (prevProps, nextProps) => {
    ...
    const prevHasExpanded = Array.from(prevProps.expandedBadges).some((id) =>
      id.startsWith(dateKey),
    );
    const nextHasExpanded = Array.from(nextProps.expandedBadges).some((id) =>
      id.startsWith(dateKey),
    );
    ...
    if (prevHasExpanded && nextHasExpanded) {
      const prevExpanded = Array.from(prevProps.expandedBadges).filter((id) =>
        id.startsWith(dateKey),
      );
      const nextExpanded = Array.from(nextProps.expandedBadges).filter((id) =>
        id.startsWith(dateKey),
      );
      if (
        JSON.stringify(prevExpanded.sort()) !==
        JSON.stringify(nextExpanded.sort())
      )
        return false;
    }
...
export default function CalendarView({
  ...
  const getJobsForDateAndProcess = useCallback(
    (date: Date, processType: string) => {
      ...
      console.log(
        `[CalendarView] Getting jobs for ${date.toDateString()} and process type: ${processType}`,
      );
      ...
      if (jobs.length > 0) {
        ...
        console.log(
          `[CalendarView] Sample requirements:`,
          (jobs[0] as any).requirements,
        );
      }
      ...
      const filteredJobs = jobs.filter((job) => {
        ...
        console.log(
          `[CalendarView] Checking job ${job.job_number}, requirements:`,
          jobRequirements,
        );
        ...
        const hasProcessType =
          ...
          jobRequirements.some((req: any) => {
            console.log(
              `[CalendarView]   - Checking requirement:`,
              req,
              `looking for process_type: ${processType}`,
            );
            if (!req.process_type) {
            ...
            console.log(
              `[CalendarView]   - Normalized process_type: ${normalized}`,
            );

            // Normalize process type variations (matching calendarUtils.ts logic)
            if (processType === "insert" && normalized === "insert")
              return true;
            ...
            if (processType === "hpPress" && normalized === "hp press")
              return true;
        ...
        console.log(
          `[CalendarView] Job ${job.job_number} hasProcessType: ${hasProcessType}`,
        );

        if (jobOverlapsDay && hasProcessType) {
          console.log(
            `[CalendarView] ✓ Job ${job.job_number} MATCHES - process: ${processType}`,
          );
        }
      ...
      console.log(
        `[CalendarView] Found ${filteredJobs.length} jobs for ${processType}`,
      );
      if (filteredJobs.length > 0) {
        console.log(
          `[CalendarView] Filtered jobs:`,
          filteredJobs.map((j) => ({
            id: j.id,
            number: j.job_number,
            name: j.job_name,
