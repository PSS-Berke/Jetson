...
export default function ProjectionsPDFHeader({
  ...
  const hasFilters = selectedClients.length > 0 || selectedServiceTypes.length > 0 || searchQuery || facility !== null || scheduleFilter !== 'all';
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center">
            ...
            <div>
              ...
              <div className="text-sm font-semibold text-gray-900">{formatStartDate()}</div>
            </div>
          </div>
          <div className="flex items-center">
            <div className="bg-blue-100 rounded-lg p-3 mr-3">
            ...
            <div>
              <div className="text-xs text-gray-500 uppercase tracking-wide">View Type</div>
              <div className="text-sm font-semibold text-gray-900">
                {granularity === 'weekly' ? 'Weekly' : granularity === 'monthly' ? 'Monthly' : 'Quarterly'}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {selectedServiceTypes.length > 0 && (
              <div className="bg-white rounded px-3 py-2 col-span-2">
                ...
                <span className="ml-2 text-blue-800 font-semibold">{selectedServiceTypes.join(', ')}</span>
              </div>
            )}
            {searchQuery && (
              <div className="bg-white rounded px-3 py-2">
            ...
            {scheduleFilter !== 'all' && (
              <div className="bg-white rounded px-3 py-2">
                ...
                <span className="ml-2 text-blue-800 font-semibold">{getScheduleFilterLabel()}</span>
              </div>
            )}
            <div className="bg-white rounded px-3 py-2">
