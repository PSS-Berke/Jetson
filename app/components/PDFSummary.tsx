...
export default function PDFSummary({
  ...
  return (
    <div className="pdf-summary">
      {/* Main Statistics */}
      <h3 className="font-semibold text-[var(--text-dark)] mb-4 mt-6">
        Summary Statistics
      </h3>
      ...
      <div className="border border-[var(--border)] rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-2 text-left text-xs font-medium text-[var(--text-dark)] uppercase">
                Process Type
              </th>
              <th className="px-4 py-2 text-center text-xs font-medium text-[var(--text-dark)] uppercase">
                Jobs
              </th>
              <th className="px-4 py-2 text-center text-xs font-medium text-[var(--text-dark)] uppercase">
                Pieces
          ...
          <tbody className="divide-y divide-[var(--border)]">
            {Object.entries(processTypeCounts).map(
              ([key, value]) =>
                value.jobs > 0 && (
                  <tr key={key}>
                    <td className="px-4 py-2 text-[var(--text-dark)]">
                      {processTypeLabels[key]}
                    </td>
                    <td className="px-4 py-2 text-center font-medium text-[var(--text-dark)]">
                      {formatNumber(value.jobs)}
                    </td>
                    <td className="px-4 py-2 text-center font-medium text-[var(--text-dark)]">
                      {formatNumber(value.pieces)}
                    </td>
                  </tr>
                ),
            )}
            {Object.values(processTypeCounts).every((v) => v.jobs === 0) && (
              <tr>
                <td
                  colSpan={3}
                  className="px-4 py-4 text-center text-[var(--text-light)]"
                >
