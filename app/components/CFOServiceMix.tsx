...
export default function CFOServiceMix({
  ...
  const pieData = sortedData.map(service => ({
    ...
    value: sortBy === 'volume' ? service.quantity : sortBy === 'revenue' ? service.revenue : service.profit,
        ...
        <div className="bg-white p-4 rounded-lg shadow-lg border border-gray-200">
          ...
          <div className="text-sm space-y-1">
            ...
            <div>
              ...
              <span className="font-semibold">{formatNumber(data.quantity)} pcs</span>
            </div>
            <div>
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
      <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Process Mix by {metricName}</h3>
          <p className="text-sm text-gray-500">Distribution analysis by process type</p>
        </div>
            ...
            >
              ...
              >
                {sortedData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={getProcessColor(entry.serviceType)} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
            ...
            <PieChart>
              ...
              >
                {pieData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={getProcessColor(entry.name)} />
                ))}
              </Pie>
              <Tooltip content={<CustomTooltip />} />
              <Legend
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-200">
        ...
        <div>
          ...
          <div className="text-sm font-semibold text-gray-900 truncate" title={sortedData[0]?.serviceType}>
        ...
        <div>
          ...
          <div className="text-sm font-semibold text-gray-900">
            {sortBy === 'revenue'
              ? formatCurrency(sortedData.reduce((sum, s) => sum + s.revenue, 0) / sortedData.length)
              : sortBy === 'volume'
              ? formatNumber(sortedData.reduce((sum, s) => sum + s.quantity, 0) / sortedData.length)
              : formatCurrency(sortedData.reduce((sum, s) => sum + s.profit, 0) / sortedData.length)
            }
          </div>
        </div>
      </div>
