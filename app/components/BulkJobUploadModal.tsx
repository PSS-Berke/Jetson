...
export default function BulkJobUploadModal({ isOpen, onClose, onSuccess }: BulkJobUploadModalProps) {
  ...
  const [uploadResults, setUploadResults] = useState<{ success: Job[]; failures: { job: Partial<Job>; error: string }[] } | null>(null);
  ...
  const handleUpload = async () => {
    ...
    const jobsToCreate: Partial<Job>[] = validJobs.map(pj => {
      ...
      const totalBilling = pj.requirements.length > 0
        ? pj.requirements.reduce((sum, req) => sum + ((pj.quantity / 1000) * req.price_per_m), 0).toFixed(2)
        : '0';
      ...
      return {
        ...
        facilities_id: pj.facility ? (pj.facility.toLowerCase().includes('lemont') || pj.facility.toLowerCase().includes('shakopee') ? 2 : 1) : 1,
        ...
        daily_split,
        confirmed: pj.schedule_type ? pj.schedule_type.toLowerCase().includes('hard') : false,
      };
    });
    ...
    switch (field) {
      ...
      case 'sub_client':
        ...
        if (value && !subClients.includes(value)) {
          ...
          setClientMappings([...clientMappings, { subClient: value, clientId: null, subClientName: value }]);
        }
        break;
      case 'description':
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600 mb-4">
                  Upload a CSV or Excel file containing your jobs. Download the template if you need a starting point.
                </p>
                <button
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                {clientMappings.map((mapping, index) => (
                  <div key={mapping.subClient} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            newMappings[index].clientId = e.target.value ? parseInt(e.target.value) : null;
          ...
          {step === 'preview' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Preview Jobs</h3>
                  <p className="text-sm text-gray-600">
                    Review the jobs before uploading. Jobs with errors will be skipped.
                  </p>
                </div>
                    ...
                    {parsedJobs.map((job, index) => {
                      ...
                      return (
                        <div key={index} className={`border rounded-lg ${hasErrors ? 'border-red-300 bg-red-50' : hasWarnings ? 'border-yellow-300 bg-yellow-50' : 'border-gray-200 bg-white'}`}>
                          {/* Collapsed Row */}
                          <div className="flex items-center justify-between p-3 cursor-pointer hover:bg-opacity-75" onClick={() => toggleRowExpansion(index)}>
                            ...
                            {(hasErrors || hasWarnings) && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setEditingJob(editingJob === index ? null : index);
                                }}
                            ...
                            <div className="border-t border-gray-200 p-4 bg-white">
                              ...
                              {editingJob === index && (
                                <div className="space-y-4 pt-4 border-t border-gray-200">
                                  <p className="font-semibold text-gray-900">Edit Job Details:</p>
                                  <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                      <label className="block text-sm font-medium text-gray-700 mb-1">Sub-Client</label>
                                      <input
                                        type="text"
                                        value={job.sub_client}
                                        onChange={(e) => handleEditJob(index, 'sub_client', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                      />
                                    </div>
                                    <div>
                                      ...
                                      <select
                                        ...
                                        onChange={(e) => handleEditJob(index, 'facility', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                      >
                                    ...
                                    <div>
                                      ...
                                      <select
                                        value={job.schedule_type || 'Soft Schedule'}
                                        onChange={(e) => handleEditJob(index, 'schedule_type', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                      >
                                    ...
                                    <div>
                                      ...
                                      <input
                                        ...
                                        onChange={(e) => handleEditJob(index, 'description', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                      />
                                    </div>
                                    <div>
                                      ...
                                      <input
                                        type="date"
                                        value={job.start_date ? job.start_date.toISOString().split('T')[0] : ''}
                                        onChange={(e) => handleEditJob(index, 'start_date', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                      />
                                    </div>
                                    <div>
                                      ...
                                      <input
                                        type="date"
                                        value={job.end_date ? job.end_date.toISOString().split('T')[0] : ''}
                                        onChange={(e) => handleEditJob(index, 'end_date', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                                      />
                                    </div>
                                  </div>
              ...
              <div className="flex justify-end gap-3">
                <button
                  onClick={() => setStep(subClients.length > 0 ? 'mapping' : 'upload')}
                  className="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors font-medium"
                >
                  Back
                </button>
          ...
          {step === 'uploading' && (
            <div className="flex flex-col items-center justify-center py-12 space-y-6">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Uploading Jobs...</h3>
                <p className="text-sm text-gray-600">
                  {uploadProgress.current} of {uploadProgress.total} jobs processed
                </p>
              </div>
          ...
          {step === 'complete' && uploadResults && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">
                  {uploadResults.success.length} job{uploadResults.success.length !== 1 ? 's' : ''} created successfully
              ...
              {uploadResults.failures.length > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                  <p className="font-semibold text-red-800 mb-2">Failed Jobs:</p>
