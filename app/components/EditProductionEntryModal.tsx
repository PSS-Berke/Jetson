...
export default function EditProductionEntryModal({
  ...
  const variance = actualQuantity && comparison
    ? parseInt(actualQuantity) - comparison.projected_quantity
    : 0;
  const variancePercentage = actualQuantity && comparison && comparison.projected_quantity > 0
    ? ((parseInt(actualQuantity) - comparison.projected_quantity) / comparison.projected_quantity) * 100
    : 0;
    ...
    const loadExistingEntry = async () => {
      if (isOpen && comparison) {
        try {
          ...
          const entries = await getProductionEntries(facilitiesId, startDate, endDate);
          ...
          console.error('[EditProductionEntryModal] Error loading entry:', error);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (existingEntryId) {
        ...
        console.log('[EditProductionEntryModal] Updating entry', existingEntryId, 'with quantity:', quantity);
        ...
        // Create new entry
        console.log('[EditProductionEntryModal] Creating new entry for job:', comparison.job.id);
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Failed to save production entry');
      ...
      <div className="relative bg-white rounded-lg shadow-xl max-w-md w-full">
          ...
          <form onSubmit={handleSubmit}>
            <div className="px-6 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={actualQuantity ? parseInt(actualQuantity).toLocaleString() : ''}
              ...
              {actualQuantity && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      <span className="text-gray-700 font-medium">Variance %:</span>
                      <span className={`ml-2 font-bold ${variancePercentage >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3">
              ...
              >
                {submitting ? 'Saving...' : existingEntryId ? 'Update Entry' : 'Create Entry'}
              </button>
            </div>
          </form>
        </div>
