...
import { ValidatedProductionRow, ValidationSummary } from '@/lib/productionValidator';
 ...
 */
export default function ExcelPreviewTable({ validatedRows, summary, onRemoveRow }: ExcelPreviewTableProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
        <div className="text-sm text-gray-600">
          Total Production Quantity: <span className="font-bold text-gray-900">{summary.totalQuantity.toLocaleString()}</span> units
        </div>
      </div>
                    ...
                    >
                      ...
                      <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-700">
                        {row.productionEntry?.date
                          ? new Date(row.productionEntry.date).toLocaleDateString()
                          : '-'}
                      </td>
                        ...
                        <td colSpan={onRemoveRow ? 8 : 7} className="px-3 py-2">
                          <div className="space-y-1">
                            {row.validation.errors.map((error, errorIndex) => (
                              <div key={`error-${errorIndex}`} className="flex items-start text-xs text-red-700">
                            ...
                            {row.validation.warnings.map((warning, warningIndex) => (
                              <div key={`warning-${warningIndex}`} className="flex items-start text-xs text-yellow-700">
                                <svg className="w-4 h-4 mr-1 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                                  <path
                                    fillRule="evenodd"
                                    d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
