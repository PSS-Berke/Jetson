    ...
    <div className="space-y-6">
      ...
      <section className="bg-white rounded-lg shadow p-6 border border-gray-200">
        ...
        <div className="space-y-3 mb-6">
              ...
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-red-700 mt-1">
                    {metrics.jobsAtRisk} job{metrics.jobsAtRisk > 1 ? 's are' : ' is'} at risk with {metrics.revenueAtRisk ? `$${metrics.revenueAtRisk.toLocaleString()}` : '$0'} in revenue at stake. Review profit margins and costs.
                  </p>
                </div>
                <span className="text-red-400 text-xs ml-2">Click for details →</span>
              </div>
            </div>
          )}
          ...
          {(() => {
            const highRiskClients = clientRevenues.filter(c => c.percentageOfTotal >= 20);
            const top3Concentration = clientRevenues.slice(0, 3).reduce((sum, c) => sum + c.percentageOfTotal, 0);
            if (highRiskClients.length > 0) {
              const highRiskJobs = highRiskClients.flatMap(client => getJobsByClient(client.clientName));
              return (
                <div
                  onClick={() => openModal(
                ...
                >
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-red-700 mt-1">
                        {highRiskClients.length} client{highRiskClients.length > 1 ? 's' : ''} represent{highRiskClients.length === 1 ? 's' : ''} ≥20% of revenue each. Top 3 clients = {top3Concentration.toFixed(1)}% of total revenue. Diversify client base to reduce dependency.
                      </p>
                    </div>
                    <span className="text-red-400 text-xs ml-2">Click for details →</span>
                  </div>
          ...
          {(() => {
            const highRiskClients = clientRevenues.filter(c => c.percentageOfTotal >= 20).length;
            const moderateRiskClients = clientRevenues.filter(c => c.percentageOfTotal >= 10 && c.percentageOfTotal < 20);
            if (moderateRiskClients.length > 0 && highRiskClients === 0) {
              const moderateRiskJobs = moderateRiskClients.flatMap(client => getJobsByClient(client.clientName));
              const totalConcentration = moderateRiskClients.reduce((sum, c) => sum + c.percentageOfTotal, 0);
              return (
                <div
                  onClick={() => openModal(
                    'Moderate Client Concentration',
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-yellow-700 mt-1">
                        {moderateRiskClients.length} client{moderateRiskClients.length > 1 ? 's' : ''} represent{moderateRiskClients.length === 1 ? 's' : ''} 10-20% of revenue. Monitor and consider diversification.
                      </p>
                    </div>
                    <span className="text-yellow-400 text-xs ml-2">Click for details →</span>
                  </div>
                </div>
              );
            }
          ...
          {serviceRevenues.length > 0 && serviceRevenues[0].percentageOfTotal > 50 && (() => {
            const primaryProcess = serviceRevenues[0];
            const processJobs = getJobsByProcessType(primaryProcess.serviceType);
            return (
              <div
                onClick={() => openModal(
                  ...
                  processJobs,
                  'process-concentration',
                  { processType: primaryProcess.serviceType, concentration: primaryProcess.percentageOfTotal }
                )}
              ...
              >
                <div className="flex items-start">
                  ...
                  <span className="text-blue-400 text-xs ml-2">Click for details →</span>
                </div>
              </div>
            );
          })()}
          ...
          {clusteringData.clusteredPeriods.length > 0 && (() => {
            ...
            const percentageText = (topCluster.percentageOfTotal * 100).toFixed(0);
            return (
              <div
                onClick={() => openModal(
                  'Job Clustering Risk',
              ...
              >
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-yellow-700 mt-1">
                      {topCluster.jobCount} jobs ({percentageText}% of total) are clustered in {topCluster.period.label}. Negotiate due date extensions or add temporary capacity to avoid delays.
                    </p>
                  </div>
                  <span className="text-yellow-400 text-xs ml-2">Click for details →</span>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div
            onClick={() => openModal(
              ...
              lowMarginData.jobs.length > 0 ? (lowMarginData.jobs as ParsedJob[]) : (jobs as ParsedJob[]),
              'profit-margin',
              { profitMargin }
            )}
            className={`p-4 rounded-lg border cursor-pointer transition-colors ${
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className={`text-xs mt-1 ${
                  ...
                  {profitMargin >= 25 ? 'Healthy margins' : profitMargin >= 15 ? 'Moderate margins' : 'Low margins - review pricing'}
                </div>
              </div>
              <span className={`text-xs ml-2 ${
                profitMargin >= 25
