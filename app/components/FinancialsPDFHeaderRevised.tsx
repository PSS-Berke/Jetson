...
export default function FinancialsPDFHeaderRevised({
  ...
  const hasFilters = selectedClients.length > 0 || selectedServiceTypes.length > 0 || searchQuery || facility !== null;
    ...
    <div className="pdf-header mb-8">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-200 rounded-lg p-5 shadow-sm">
          <div className="flex items-start">
            ...
            <div className="flex-1">
              <div className="text-xs text-blue-600 font-semibold uppercase tracking-wide mb-1">Analysis Period</div>
              <div className="text-lg font-bold text-blue-900">{formatDateRange()}</div>
              <div className="text-xs text-blue-600 mt-1">{getGranularityLabel()} Breakdown</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 border-2 border-purple-200 rounded-lg p-5 shadow-sm">
          <div className="flex items-start">
            ...
            <div className="flex-1">
              <div className="text-xs text-purple-600 font-semibold uppercase tracking-wide mb-1">Facility</div>
              <div className="text-lg font-bold text-purple-900">{getFacilityName()}</div>
              <div className="text-xs text-purple-600 mt-1">Operating Location</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-200 rounded-lg p-5 shadow-sm">
          <div className="flex items-start">
            ...
            <div className="flex-1">
              <div className="text-xs text-green-600 font-semibold uppercase tracking-wide mb-1">Report Type</div>
              <div className="text-lg font-bold text-green-900">Financial Analysis</div>
              <div className="text-xs text-green-600 mt-1">Executive Summary</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {hasFilters && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-l-4 border-orange-500 rounded-lg p-6 shadow-sm">
          <div className="flex items-start mb-4">
            ...
            <div>
              <h3 className="font-bold text-orange-900 text-base mb-1">Active Filters Applied</h3>
              <p className="text-sm text-orange-700">This report reflects filtered data based on the criteria below</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {facility !== null && (
              <div className="bg-white rounded-lg px-4 py-3 border border-orange-200 shadow-sm">
                <div className="text-xs font-semibold text-orange-600 uppercase tracking-wide mb-1">Facility Filter</div>
                <div className="text-sm text-orange-900 font-bold">{getFacilityName()}</div>
              </div>
            )}
            ...
            {searchQuery && (
              <div className="bg-white rounded-lg px-4 py-3 border border-orange-200 shadow-sm">
                <div className="text-xs font-semibold text-orange-600 uppercase tracking-wide mb-1">Search Query</div>
                <div className="text-sm text-orange-900 font-mono">&quot;{searchQuery}&quot;</div>
              </div>
            )}
            <div className="bg-white rounded-lg px-4 py-3 border border-orange-200 shadow-sm">
              <div className="text-xs font-semibold text-orange-600 uppercase tracking-wide mb-1">Filter Mode</div>
              <div className="text-sm text-orange-900 font-bold">
                {filterMode === 'AND' ? 'Match All Criteria' : 'Match Any Criteria'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-6 pt-4 border-t-2 border-gray-200">
        <div className="flex items-start text-xs text-gray-600">
          ...
          <p>
            <span className="font-semibold">Confidential:</span> This financial report contains sensitive business data.
            All figures are based on current projections and actual recorded costs.
            Data accuracy is subject to ongoing updates and reconciliation.
          </p>
        </div>
      </div>
    </div>
