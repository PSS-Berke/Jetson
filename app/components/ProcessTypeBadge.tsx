...
function hexToTailwindClasses(hexColor: string): { textColor: string; bgColor: string } {
  ...
  return colorMap[hexColor] || { textColor: 'text-gray-800', bgColor: 'bg-gray-100' };
}
...
export default function ProcessTypeBadge({ processType }: ProcessTypeBadgeProps) {
  ...
  const label = config ? getDisplayLabel(config.key, config.label) : processType;
  const colors = config ? hexToTailwindClasses(config.color) : { textColor: 'text-gray-800', bgColor: 'bg-gray-100' };
