...
import { DEFAULT_COLUMNS, InlineJobCostColumnConfig, getColumnByKey } from "./columnConfig";
...
export default function ColumnSettingsPopover({
  ...
  // Filter out batch mode only columns from the settings
  const configurableColumns = columnOrder.filter((key) => {
    const config = getColumnByKey(key);
    if (!config) return false;
    // Don't show batch mode only columns in settings (they auto-show in batch mode)
    if (config.batchModeOnly) return false;
  ...
  const canToggle = (config: InlineJobCostColumnConfig): boolean => {
    return !config.required;
  };

  const canReorder = (config: InlineJobCostColumnConfig): boolean => {
