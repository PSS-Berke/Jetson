...
export default function EditJobModal({
  ...
  useEffect(() => {
    if (isOpen && job) {
      ...
      // Initialize splitResults from job's daily_split if available
      const jobWithDailySplit = job as typeof job & {
        daily_split?: Array<{ date: string; quantity: number }> | number[][];
      };
      if (jobWithDailySplit.daily_split && Array.isArray(jobWithDailySplit.daily_split)) {
        // Check if it's the new format (array of {date, quantity}) or old 2D array format
        const firstItem = jobWithDailySplit.daily_split[0];
        if (firstItem && typeof firstItem === 'object' && 'date' in firstItem) {
          // New format: array of {date, quantity}
          const convertedResults = (jobWithDailySplit.daily_split as Array<{ date: string; quantity: number }>).map((item) => {
            const date = new Date(item.date.split('.').reverse().join('-')); // Convert DD.MM.YYYY to Date
            const dayOfWeek = date.getDay(); // 0 = Sunday
            const calendarDayInWeek = dayOfWeek === 0 ? 7 : dayOfWeek; // Convert to Mon=1, Sun=7
          ...
          setSplitResults(convertedResults);
          setLockedSplitWeeks({});
        }
      } else {
        // No daily_split, clear the split results
        setSplitResults([]);
        setLockedSplitWeeks({});
      }
    }
  ...
  // Fetch split results from API when dates/quantity change
  useEffect(() => {
    if (
      formData.start_date &&
      formData.due_date &&
