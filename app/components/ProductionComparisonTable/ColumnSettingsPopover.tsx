...
import { DEFAULT_COLUMNS, ProductionColumnConfig, getColumnByKey } from "./columnConfig";
...
export default function ColumnSettingsPopover({
  ...
  const configurableColumns = columnOrder.filter((key) => {
    ...
    // Show if reorderable OR if it can be toggled (not required)
  ...
  const canToggle = (config: ProductionColumnConfig): boolean => {
    return !config.required;
  };

  // Check if column is reorderable
  const canReorder = (config: ProductionColumnConfig): boolean => {
